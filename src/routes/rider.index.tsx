...
export const Route = createFileRoute("/rider/")({
  head: () => ({ meta: [{ title: "Rider dashboard — DashPoint" }] }),
...
function RiderDashboard() {
  ...
  function accept(id: string) {
    ...
    navigate({ to: "/rider/job/$id", params: { id } });
  }
      ...
      <main className="p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
        <section className="lg:col-span-4 space-y-6">
          <div className="rounded-2xl border border-surface-200 bg-navy text-white p-6 shadow-lg">
            ...
            <p className="mt-1 font-display text-4xl font-bold">${todayEarnings.toFixed(2)}</p>
          ...
          <div className="rounded-2xl border border-surface-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-slate-500">{incoming.length} jobs available nearby</p>
              </div>
                ...
                {incoming.map((d) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <div className={`size-2 rounded-full ${d.packageType === "Cargo" ? "bg-orange-400" : "bg-brand"}`} />
                          <span className="text-xs font-bold">{d.packageType}</span>
                          <span className="text-xs text-slate-400">· {d.distanceKm} km</span>
                        </div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[10px] text-slate-400">ETA {d.etaMinutes}m</p>
                      </div>
                    </div>
